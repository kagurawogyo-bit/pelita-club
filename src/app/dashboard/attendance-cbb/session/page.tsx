...
export default function CbbCoachSessionPage() {
  ...
  // Selection states
  const [selectedColumn, setSelectedColumn] = useState("CBB-1");
  ...
  const fetchActiveSession = async () => {
    try {
      const res = await fetch("/api/attendance/session?eventType=CBB");
      if (res.ok) {
        const data = await res.json();
        if (data && !data.error) setActiveSession(data);
      }
    } catch (err) {
  ...
  const handleCreateSession = async () => {
    if (selectedCoachIds.length === 0) {
      setError("Pilih minimal satu pelatih yang melatih event ini.");
      return;
    }
  ...
  const handleStopSession = async () => {
    try {
      await fetch("/api/attendance/session?eventType=CBB", { method: "DELETE" });
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ marginBottom: '32px' }}>
        <h2 style={{ fontSize: '2rem', fontWeight: 800 }}>🎫 Sesi QR Absen Event CBB</h2>
        <p style={{ color: 'var(--text-secondary)' }}>
          Buat sesi absen CBB khusus agar peserta bisa scan otomatis.
        </p>
      </div>

      {!activeSession ? (
        <div className="glass-card" style={{ padding: '32px' }}>
          <h3 style={{ fontWeight: 700, marginBottom: '24px' }}>Konfigurasi Sesi Event CBB</h3>
          ...
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 700, marginBottom: '12px' }}>Pilih Pelatih Bertugas (Bisa lebih dari satu)</label>
          ...
          <div style={{ marginBottom: '32px' }}>
            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 700, marginBottom: '12px' }}>Pilih Pertemuan Event CBB</label>
            <div style={{ 
              display: 'grid', 
          ...
          >
            {loading ? "Memproses..." : "Mulai Sesi & Tampilkan QR CBB"}
          </button>
        </div>
      ) : (
        <div className="glass-card qr-card-responsive">
          ...
          <div className="qr-wrapper-responsive">
            <div className="qr-svg-container">
              <QRCode 
                value={JSON.stringify({ sessionId: activeSession.id, type: "attendance_session_cbb" })} 
                size={256}
          ...
          <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)', marginBottom: '32px', maxWidth: '400px', margin: '0 auto 32px' }}>
            Minta peserta event CBB untuk membuka menu <strong>Scan Absen CBB</strong> di HP mereka dan arahkan ke kode QR ini.
          </p>
